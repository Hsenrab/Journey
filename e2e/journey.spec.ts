...
test.describe('main journey', () => {
  ...
  test('records a visit and sees it reflected across the app', async ({ page }) => {
    ...
    await page.getByLabel('Search locations').fill('May Hill')
    await expect(page.getByText('May Hill', { exact: true })).toBeVisible()
    await expect(page.getByText('Dyrham Park')).not.toBeVisible()

    await page.getByRole('link', { name: 'View details' }).click()
    await expect(page.getByRole('heading', { name: 'May Hill' })).toBeVisible()
