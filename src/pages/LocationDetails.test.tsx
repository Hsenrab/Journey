...
describe('LocationDetails', () => {
  ...
  it('shows the location details', () => {
    renderDetails('dyrham-park')
    expect(screen.getByRole('heading', { name: 'Dyrham Park' })).toBeInTheDocument()
  })
  ...
  it('adds a visit and persists the derived status', async () => {
    ...
    expect(load()['dyrham-park']).toMatchObject({ status: 'gold', notes: 'Wonderful visit' })
  })
  ...
  it('parses photo references into an array, ignoring blank lines', async () => {
    ...
    expect(load()['dyrham-park'].photos).toEqual(['a.jpg', 'b.jpg'])
  })
  ...
  it('allows editing the visit date', async () => {
    ...
    expect(load()['dyrham-park']).toMatchObject({ date: '2026-07-04' })
  })
})
