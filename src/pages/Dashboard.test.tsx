...
describe('Dashboard', () => {
  ...
  it('derives status counts per waypoint and links each to a filtered Waypoints view', () => {
    ...
    expect(getByRole('link', { name: 'Bronze' })).toHaveAttribute('href', '/waypoints?status=bronze')
    expect(getByRole('link', { name: 'Silver' })).toHaveAttribute('href', '/waypoints?status=silver')
    expect(getByRole('link', { name: 'Gold' })).toHaveAttribute('href', '/waypoints?status=gold')
  })
